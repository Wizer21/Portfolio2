...
export default {
  ...
  mounted(){
    ...
    let ground = Bodies.rectangle( -200, window.innerHeight, window.innerWidth * 2 + 800, 50, { isStatic: true })
    ...
    for (let i = 0; i < window.innerWidth/25; i++){
      let leaf = Bodies.rectangle(dice(0, window.innerWidth), dice(0, window.innerHeight), 40, 40, {        
        frictionAir: 0.2,
      })
